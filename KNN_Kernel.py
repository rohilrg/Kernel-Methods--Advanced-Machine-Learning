...
def kernilize_data(data,type_of_kernel=None,gamma=0.1,poly_constant=1,sigmoid_constant=1,sigmoid_alpha=0.1):
    ...
    if type_of_kernel == 'poly':
        ...
        for combinations in itertools.combinations(data,2):
            k = (combinations[0].T.dot(combinations[1])+poly_constant)**number_of_feature
            K.append(k)
        K = np.array(K)
        K = squareform(K)
...
for centroid in clf.centroids:
    ...
    ax.scatter(clf.centroids[0], clf.centroids[1], clf.centroids[2], c="k", cmap=plt.cm.rainbow)
